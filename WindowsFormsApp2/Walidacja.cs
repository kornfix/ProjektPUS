...
namespace WindowsFormsApp2
{
    class Walidacja
    {
        ...
        public static async Task<Error> WalidacjaEmail(String email, Control control, Control ogolna)
        {
            ...
            {
                return new Error(control, "Podany email jest nieprawidłowy!");
            }
            Odpowiedz odp = AsynchronicznyKlient.zapytaj(Pytanie.komendy.sprawdz_email,
                new object[] { email});
            if (odp.czy_wzrocono_error())
            {
                return new Error(ogolna, "Nie udane polaczenie z bazą danych!");
            }
            else if (((JsonElement)odp.Argumenty[0]).GetBoolean())
            {
        ...
        public static async Task<Error> WalidacjaLoginu(String login, Control control, Control ogolna)
        {
            ...
            Odpowiedz odp = AsynchronicznyKlient.zapytaj(Pytanie.komendy.sprawdz_login,
                new object[] { login});
            if (odp.czy_wzrocono_error())
            {
                return new Error(ogolna, "Nie udane polaczenie z bazą danych!");
        ...
        public static bool czy_bledy(System.ComponentModel.RunWorkerCompletedEventArgs e)
        {
            if (e.Error != null)
            {
            ...
            {
                MessageBox.Show("Anulowanie zadania", "Problem");
            }
